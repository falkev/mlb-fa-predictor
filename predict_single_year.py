...
OUTPUT_PATH     = "2026_predictions_single_year.csv"
RANDOM_SEED     = 42
...
# FEATURE SETS PER GROUP — single year only, no _2yr / _3yr / _trend columns
...
def plot_pred_vs_actual(signed_preds, save_path="2026_pred_vs_actual_single_year.png"):
    df = signed_preds[signed_preds["Actual_AAV"].notna()].copy()
...
def plot_feature_importance(models, feat_cols, top_n=12):
    for group, model in models.items():
        ...
        plt.tight_layout()
        save_path = f"importance_2026_{group}_single_year.png"
        plt.savefig(save_path, dpi=150)
        print(f"  Saved: {save_path}")
...
def main():
    print("=== 2026 MLB FA Salary Predictor (Position-Group Models, Single Year Stats) ===\n")
    ...
    if len(signed_preds) > 0:
        plot_top_predictions(signed_preds,
                             ...
                             "2026_signed_predictions_single_year.png")
    if len(unsigned_preds) > 0:
        plot_top_predictions(unsigned_preds,
                             "Top Unsigned 2026 FAs — Predicted AAV",
                             "2026_unsigned_predictions_single_year.png")
